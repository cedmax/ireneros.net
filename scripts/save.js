...
async function saveImage(prefix, image, name) {
  ...
  const filename = name
    ? `${name}${data.fileName.match(/\.[0-9a-z]+$/i)[0]}`
    : data.fileName;
  const options = {
    url: `http:${data.url}`,
...
async function creation(content, type) {
  ...
  for (const item of contentfulContent) {
    ...
    if (item.video) {
      const image = await saveImage(
        item.slug,
        item.videoThumbnail,
        item.video.replace("https://vimeo.com/", "")
      );
  ...
  fileContent.menu = Object.keys(fileContent.menu)
    .map(key => fileContent.menu[key])
...
module.exports = async content => {
  ...
  for (const section of index.sections) {
    const sectionImage = await saveImage(
      "home",
      metadata[`${section.slug}Image`]
    );
